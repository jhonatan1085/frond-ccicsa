...
export class InvoicesCancelledComponent implements OnInit {
  ...
  private getTableData(): void {
    ...
    this.data.getInvoicesCancelled().subscribe((data: apiResultFormat) => {
  ...
  public sortData(sort: Sort) {
    const data = this.invoicesCancelled.slice();

    if (!sort.active || sort.direction === '') {
      this.invoicesCancelled = data;
    } else {
      this.invoicesCancelled = data.sort((a, b) => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
