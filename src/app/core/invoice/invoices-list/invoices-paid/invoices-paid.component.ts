...
export class InvoicesPaidComponent implements OnInit {
  ...
  private getTableData(): void {
    ...
    this.data.getInvoicesPaid().subscribe((data: apiResultFormat) => {
      ...
      this.dataSource = new MatTableDataSource<invoicespaid>(this.invoicesPaid);
  ...
  public searchData(value: any): void {
    ...
    this.invoicesPaid = this.dataSource.filteredData;
  }

  public sortData(sort: Sort) {
    const data = this.invoicesPaid.slice();

    if (!sort.active || sort.direction === '') {
      this.invoicesPaid = data;
    } else {
      this.invoicesPaid = data.sort((a, b) => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
