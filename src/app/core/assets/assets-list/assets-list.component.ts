...
export class AssetsListComponent implements OnInit {
    ...
    this.data.getAssetsList().subscribe((data: apiResultFormat) => {
      ...
      data.data.map((res: assetsList, index: number) => {
        ...
        if (index >= this.skip && serialNumber <= this.limit) {
          this.assetsList.push(res);
          this.serialNumberArray.push(serialNumber);
        }
      });
      this.dataSource = new MatTableDataSource<assetsList>(this.assetsList);
  ...
  public searchData(value: any): void {
    ...
    this.assetsList = this.dataSource.filteredData;
  }

  public sortData(sort: Sort) {
    const data = this.assetsList.slice();
